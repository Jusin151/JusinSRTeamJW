...
HRESULT CRenderer::Render_Blend()
{
	...
	//static_cast<CTransform*>(CGameInstance::Get_Instance()->Find_Object(3, L"Layer_Camera")->Get_Component(L"Com_Transform"))->Get_State(CTransform::STATE_POSITION);
	if (!m_RenderObjects[RG_BLEND].empty())
	{
		CTransform* cameraTransform = static_cast<CTransform*>(CGameInstance::Get_Instance()->Find_Object(3, L"Layer_Camera")->Get_Component(L"Com_Transform"));
	...
	//sort(m_RenderObjects->begin(), m_RenderObjects->end(), [cameraTransform](CGameObject* a, CGameObject* b) -> bool {
	//	// 입력 유효성 검사 (선택 사항이지만 권장)
	//	if (!cameraTransform) return false;
	//	if (!a || !b) return false; // 혹은 다른 규칙 적용 (예: nullptr을 뒤로 보내기)
	//	CTransform* transformA = static_cast<CTransform*>(a->Get_Component(L"Com_Transform"));
	...
	//	_float3 CtoA = posA - posC;
	//	_float3 CtoB = posB - posC;
	//	
	//	_float distSqA = D3DXVec3LengthSq(&CtoA); // 아래 Helper 함수 참조
	//	_float distSqB = D3DXVec3LengthSq(&CtoB); // 아래 Helper 함수 참조
