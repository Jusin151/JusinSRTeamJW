...
public:
	...
	_float3 Get_State(CTransform::TRANSFORMSTATE eState) const { return *reinterpret_cast<const _float3*>(&m_WorldMatrix.m[eState][0]); }
	void	Set_State(CTransform::TRANSFORMSTATE eState, const _float3& vState) { memcpy(&m_WorldMatrix.m[eState][0], &vState, sizeof vState); }
	void	Set_WorldMat(_float4x4 worldMat) { m_WorldMatrix = worldMat; }
	COLLIDERGROUP	Get_Type() { return m_eType; }
	void			Set_Type(COLLIDERGROUP eType) { m_eType = eType; }
	COLLIDERGROUP	Get_Other_Type() { return m_eOtherType; }
	void	Set_Other_Type(COLLIDERGROUP eOtherType) { m_eOtherType = eOtherType; }
	_float	Get_Radius() const  { return max(max(m_fScale.x, m_fScale.y), m_fScale.z) * 0.5f; }
