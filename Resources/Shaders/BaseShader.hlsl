...
struct Light
{
    ...
    // --- 스포트라이트용 추가 정보 (필요시) ---
    float Falloff; // 스포트라이트 감쇠 지수 (Power)
    float Theta; // 스포트라이트 내부 각 코사인값
    float Phi; // 스포트라이트 외부 각 코사인값
    float _unused2_;
};
...
PS_OUT PS_PORTAL(PS_IN In, float facing : VFACE)
{
    ...
    // 2. 텍스처의 RGB와 재질의 Diffuse RGB를 곱하여 기본 색상 조절
    //    (재질의 알파는 최종 알파 설정에만 사용)
    baseColor *= g_Material.Diffuse;
    Out.vColor = baseColor;
    
    // 3. 최종 색상 설정
    //    RGB는 위에서 계산한 값을 사용하고,
    //    Alpha는 재질의 알파 값(g_Material.Diffuse.a)을 직접 사용
    //Out.vColor = float4(baseColor.rgb, g_Material.Diffuse.a);

    // 4. (선택 사항) 알파 값에 따른 discard 로직
    //    이제 discard 조건은 재질의 알파 값에 따라 결정됩니다.
    if (Out.vColor.r > 0.5f &&
        Out.vColor.g > 0.5f &&
        Out.vColor.b > 0.5f)
        discard;
    if (Out.vColor.a < 0.5f) // 필요에 따라 이 임계값을 조절하세요.
       discard;
...
technique DefaultTechnique
{
    ...
    pass PORTAL
    {
        ...
        // --- 셰이더 설정 ---
        VertexShader = compile vs_3_0 VS_MAIN();
        PixelShader = compile ps_3_0 PS_PORTAL(); // 빛 계산이 포함된 PS_MAIN 사용
    }
